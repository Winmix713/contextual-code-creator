...
import LeagueComparisonRadarChart, { type LeagueRadarMetric } from "@/components/crossleague/LeagueComparisonRadarChart"
...
const CrossLeague = () => {
  ...
  const radarData: LeagueRadarMetric[] = useMemo(() => {
    ...
    return (analyzeQuery.data.leagues as LeagueRow[]).map((l: LeagueRow) => {
      const metricsObj = analyzeQuery.data!.metrics[l.id] ?? {
        goals: 0,
        home_adv: 0,
        balance: 0,
        predictability: 0,
      ...
      Object.entries(metricsObj).forEach(([key, val]) => {
        numericMetrics[key] = typeof val === "number" ? val : 0
      })
      return {
        leagueId: l.id,
        leagueName: l.name,
        metric: numericMetrics.goals ?? 0,
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {(leaguesQuery.data ?? []).map((l) => (
                <Badge
                  key={l.id}
                  onClick={() =>
                    setSelected((prev) => (prev.includes(l.id) ? prev.filter((x) => x !== l.id) : [...prev, l.id]))
                  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <LeagueComparisonRadarChart data={radarData} leagues={radarData} />
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Network className="w-4 h-4" /> CorrelationHeatmap (scoring_trend)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <CorrelationHeatmap data={heatmap.matrix} leagues={heatmap.leagues} />
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(metaPatternsQuery.data?.meta_patterns ?? []).map((mp: MetaPatternRow) => (
                  <div
                    key={`${mp.pattern_name}-${mp.pattern_type}`}
                    className="p-4 rounded-lg ring-1 ring-border bg-card"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="font-semibold">{mp.pattern_name}</div>
                        <div className="text-xs text-muted-foreground mt-1">
                          Típus: {mp.pattern_type} - Evidence: {mp.evidence_strength}%
                        </div>
